<script setup>
  ...
  const generateAiSummary = async (isBackground = false) => {
    ...
    if (!isBackground) aiResponse.value = ''; 
    aiError.value = '';
    ...
    else if (tab.value === 'meals') { type = 'meals'; }
    else if (tab.value === 'birthdays') { type = 'birthdays'; }
    else if (tab.value === 'christmas') { type = 'christmas'; }
    else if (tab.value === 'sandbox') { type = 'sandbox'; }
    else if (tab.value === 'mortgage') { type = 'mortgage'; }
    else { type = tab.value; }
  ...
  const handleAiClick = () => {
      ...
      if (showAiDialog.value && !aiResponse.value && !aiLoading.value) {
  ...
  const popOutCalculator = () => { const w = window.open("", "Calculator", "width=320,height=480,resizable=yes"); if(!w) return showMsg("Pop-up blocked!", "error"); const content = `<html><head><title>Money Calc</title><style>body{font-family:-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; display:flex;flex-direction:column;height:100vh;margin:0;padding:10px;background:#121212;box-sizing:border-box; color: white;} input{width:100%;font-size:32px;text-align:right;padding:15px;margin-bottom:15px;border-radius:8px;border:1px solid #333;background:#1e1e1e;color:white;box-sizing:border-box;} .grid{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;flex:1;} button{font-size:24px;border-radius:8px;border:none;cursor:pointer;background:#2d2d2d;color:white;transition:0.1s;} button:active{background:#404040;} .primary{background:#1976D2;}.primary:active{background:#1565C0;} .danger{background:#CF6679;color:#000;}</style></head><body><input type="text" id="display" readonly value="0"><div class="grid"><button onclick="app('7')">7</button><button onclick="app('8')">8</button><button onclick="app('9')">9</button><button class="primary" onclick="app('/')">÷</button><button onclick="app('4')">4</button><button onclick="app('5')">5</button><button onclick="app('6')">6</button><button class="primary" onclick="app('*')">×</button><button onclick="app('1')">1</button><button onclick="app('2')">2</button><button onclick="app('3')">3</button><button class="primary" onclick="app('-')">-</button><button class="danger" onclick="clr()">C</button><button onclick="app('0')">0</button><button onclick="app('.')">.</button><button class="primary" onclick="app('+')">+</button><button class="primary" style="grid-column:span 4" onclick="res()">=</button></div><script>let d = document.getElementById('display');function app(v){ if(d.value==='0' && !['/','*','-','+'].includes(v)) d.value=v; else d.value+=v; }function clr(){ d.value='0'; }function res(){ try{d.value=eval(d.value)}catch(e){d.value='Err'} }document.addEventListener('keydown', (e) => { const k = e.key; if(/[0-9.+\\-*/]/.test(k)) app(k); if(k==='Enter'||k==='=') res(); if(k==='Backspace') d.value = d.value.slice(0,-1)||'0'; if(k.toLowerCase()==='c') clr(); });<\/script></body></html>`; w.document.write(content); showCalculator.value = false; };
  ...
  const fetchSettings = async () => {
    ...
    } catch (err) { console.error(err); showMsg("Failed to load settings", "error"); }
  };
    ...
    <v-app>
      <v-navigation-drawer v-model="drawer" :temporary="mobile" color="surface">
        ...
        <v-list nav density="compact" class="mt-2" v-model:opened="openList">
          ...
          <v-list-group value="reports_group">
            <template v-slot:activator="{ props }"><v-list-item v-bind="props" prepend-icon="mdi-chart-box-outline" title="Reports" rounded="xl"></v-list-item></template>
            ...
            <v-list-item v-if="aiEnabled" prepend-icon="mdi-robot-outline" title="AI Summary" value="ai_summary" @click="selectTab('ai_summary')" :active="tab === 'ai_summary'" color="primary" rounded="xl"></v-list-item>
          </v-list-group>
  
          <v-list-group value="admin_group">
            <template v-slot:activator="{ props }"><v-list-item v-bind="props" prepend-icon="mdi-shield-account-outline" title="Admin" rounded="xl"></v-list-item></template>
          ...
          <v-list-group value="settings_group">
            <template v-slot:activator="{ props }"><v-list-item v-bind="props" prepend-icon="mdi-cog-outline" title="Settings" rounded="xl"></v-list-item></template>
      ...
      <v-app-bar color="surface" density="compact" flat border>
        ...
        <v-btn 
            v-if="isAiAvailable"
            icon 
            :color="aiHasResponse ? 'amber-darken-2' : 'primary'" 
        ...
        >
            ...
            <template v-else>
                <v-badge dot color="amber" v-if="aiHasResponse && !showAiDialog">
      ...
      <v-card v-if="showSavings" class="position-fixed rounded-lg border" style="z-index: 9998; width: 300px; max-height: 500px; overflow: hidden; display: flex; flex-direction: column;" :style="{ left: savingsPos.x + 'px', top: savingsPos.y + 'px' }" elevation="12"><div class="bg-primary text-white d-flex align-center justify-space-between pa-2 cursor-move" @mousedown="startDragSavings"><span class="text-subtitle-2 font-weight-bold ml-2"><v-icon size="small" class="mr-1">mdi-piggy-bank</v-icon> Savings</span><v-btn icon="mdi-close" variant="text" density="compact" size="small" color="white" @click="showSavings = false"></v-btn></div><div class="overflow-y-auto pa-0 flex-grow-1 bg-surface"><v-expansion-panels variant="accordion"><v-expansion-panel v-for="acc in savingsAccounts" :key="acc.id"><v-expansion-panel-title class="py-1 min-height-40">{{ acc.name }} <template v-slot:actions="{ expanded }"><span class="text-caption font-weight-bold mr-2">£{{ acc.total.toFixed(0) }}</span><v-icon :icon="expanded ? 'mdi-chevron-up' : 'mdi-chevron-down'"></v-icon></template></v-expansion-panel-title><v-expansion-panel-text class="pa-0"><v-list density="compact" class="pa-0"><v-list-item v-for="pot in acc.pots" :key="pot.id" class="border-b pl-2 pr-2 no-drag"><div class="d-flex align-center justify-space-between width-100"><span class="text-caption text-truncate mr-2" style="max-width: 100px" :title="pot.name">{{ pot.name }}</span><v-text-field :model-value="pot.amount" @update:model-value="v => { pot.amount = Number(v); updatePot(pot) }" type="number" prefix="£" density="compact" variant="plain" hide-details class="text-right text-caption font-weight-bold" style="width: 80px"></v-text-field></div></v-list-item></v-list></v-expansion-panel-text></v-expansion-panel></v-expansion-panels></div></v-card>
      <v-card v-if="showCalculator" width="300" elevation="12" border class="position-fixed rounded-lg" style="z-index: 9999;" :style="{ left: calcPos.x + 'px', top: calcPos.y + 'px' }"><v-card-title class="bg-primary text-white d-flex justify-space-between align-center py-2 text-subtitle-1 cursor-move" @mousedown="startDragCalc">Calculator <div><v-btn icon="mdi-open-in-new" variant="text" density="compact" color="white" @click="popOutCalculator" class="mr-1" title="Pop out"></v-btn><v-btn icon="mdi-close" variant="text" density="compact" color="white" @click="showCalculator = false"></v-btn></div></v-card-title><v-card-text class="pa-2 bg-surface"><v-text-field v-model="calcDisplay" variant="outlined" hide-details class="text-right mb-2 text-h5 font-weight-bold" readonly bg-color="surface"></v-text-field><v-row dense><v-col cols="3" v-for="btn in ['7','8','9','/', '4','5','6','*', '1','2','3','-', 'C','0','.','+']" :key="btn"><v-btn block height="45" :color="['C'].includes(btn) ? 'error' : ['/','*','-','+'].includes(btn) ? 'secondary' : undefined" variant="flat" class="text-h6 no-drag" @click="btn === 'C' ? calcClear() : calcAppend(btn)">{{ btn }}</v-btn></v-col><v-col cols="12"><v-btn block color="primary" height="45" class="text-h6 no-drag" @click="calcResult">=</v-btn></v-col></v-row></v-card-text></v-card>
      <v-card v-if="showAiDialog" class="position-fixed rounded-lg border" style="z-index: 9999; width: 500px; max-height: 600px; display: flex; flex-direction: column;" :style="{ left: aiPos.x + 'px', top: aiPos.y + 'px' }" elevation="12"><v-card-title class="bg-primary text-white d-flex align-center justify-space-between cursor-move" @mousedown="startDragAi"><span class="text-truncate">AI Insight: {{ tab.toUpperCase().replace('_', ' ') }}</span><div class="d-flex"><v-btn icon="mdi-refresh" variant="text" density="compact" color="white" class="mr-1" @click="generateAiSummary(false)" title="Refresh"></v-btn><v-btn icon="mdi-minus" variant="text" density="compact" color="white" class="mr-1" @click="showAiDialog = false" title="Minimize"></v-btn><v-btn icon="mdi-close" variant="text" density="compact" color="white" @click="showAiDialog = false" title="Close"></v-btn></div></v-card-title><v-card-text class="pa-4 flex-grow-1 overflow-y-auto no-drag" style="min-height: 200px"><div v-if="aiLoading" class="d-flex flex-column align-center justify-center py-8"><v-progress-circular indeterminate color="primary" size="64"></v-progress-circular><div class="mt-4 text-caption text-grey">Analyzing data...</div></div><div v-else-if="aiError" class="text-center text-error py-4"><v-icon color="error" size="large" class="mb-2">mdi-alert-circle</v-icon><div>{{ aiError }}</div></div><div v-else style="white-space: pre-wrap; line-height: 1.6">{{ aiResponse }}</div></v-card-text></v-card>
      <v-snackbar v-model="snackbar" :color="snackbarColor" timeout="3000" location="bottom right">{{ snackbarText }}<template v-slot:actions><v-btn variant="text" @click="snackbar = false">Close</v-btn></template></v-snackbar>
    </v-app>
  </template>
