...
const editableHeaders = ref([]); // Local headers for drag-and-drop

const schemas = {
    expenses: [
        { title: 'ID', key: 'id', align: 'start', readonly: true, width: '50px' },
        { title: 'Month', key: 'month', type: 'text', width: '100px' },
        { title: 'Name', key: 'name', type: 'text', width: '200px' },
        { title: 'Amount', key: 'amount', type: 'number', width: '100px' },
        { title: 'Category', key: 'category', type: 'text', width: '150px' },
        { title: 'Who', key: 'who', type: 'text', width: '100px' },
        { title: 'Paid', key: 'paid', type: 'boolean', width: '80px' },
    ],
    expense_templates: [
        { title: 'ID', key: 'id', readonly: true, width: '50px' },
        { title: 'Name', key: 'name', type: 'text', width: '200px' },
        { title: 'Default Amount', key: 'amount', type: 'number', width: '120px' },
        { title: 'Category', key: 'category', type: 'text', width: '150px' },
        { title: 'Who', key: 'who', type: 'text', width: '100px' },
    ],
    settings: [
        { title: 'Key', key: 'key', type: 'text', readonly: true, width: '200px' },
        { title: 'Value', key: 'value', type: 'text', width: 'auto' },
    ],
    savings_accounts: [
        { title: 'ID', key: 'id', readonly: true, width: '50px' },
        { title: 'Account Name', key: 'name', type: 'text', width: 'auto' },
    ],
    savings_pots: [
        { title: 'ID', key: 'id', readonly: true, width: '50px' },
        { title: 'Account ID', key: 'account_id', type: 'number', width: '100px' },
        { title: 'Pot Name', key: 'name', type: 'text', width: '200px' },
        { title: 'Amount', key: 'amount', type: 'number', width: '120px' },
    ],
    christmas_list: [
        { title: 'ID', key: 'id', readonly: true, width: '50px' },
        { title: 'Recipient', key: 'recipient', type: 'text', width: '150px' },
        { title: 'Item', key: 'item', type: 'text', width: '200px' },
        { title: 'Amount', key: 'amount', type: 'number', width: '100px' },
        { title: 'Bought', key: 'bought', type: 'boolean', width: '80px' },
    ],
    sandbox_expenses: [
        { title: 'ID', key: 'id', readonly: true, width: '50px' },
        { title: 'Name', key: 'name', type: 'text', width: '200px' },
    ...
    credit_cards: [
        { title: 'ID', key: 'id', readonly: true, width: '50px' },
        { title: 'Name', key: 'name', type: 'text', width: '200px' },
        { title: 'Limit', key: 'limit_amount', type: 'number', width: '120px' },
        { title: 'APR %', key: 'interest_rate', type: 'number', width: '80px' },
        { title: 'Balance', key: 'balance', type: 'number', width: '120px' },
    ],
    cc_transactions: [
        { title: 'ID', key: 'id', readonly: true, width: '50px' },
        { title: 'Card ID', key: 'card_id', type: 'number', readonly: true, width: '80px' },
        { title: 'Date', key: 'date', type: 'text', width: '120px' },
        { title: 'Desc', key: 'description', type: 'text', width: '200px' },
        { title: 'Amount', key: 'amount', type: 'number', width: '100px' },
        { title: 'Paid', key: 'paid', type: 'boolean', width: '80px' },
    ],
};
...
const salaryColumns = ref([
    ...
    { key: 'month', label: 'Month', align: 'left', width: '150px', sortable: true },
...
const addSalaryEntry = async () => {
    ...
    fetchSalaryData(); emit('notify', 'Entry Added');
};
...
const saveSalaryRow = async () => {
    ...
    editingId.value = null; fetchSalaryData(); emit('notify', 'Data Saved');
};
...
const deleteSalarySelected = async () => {
    ...
    try { await Promise.all(selectedSalaryRows.value.map(m => axios.delete(`${API_URL}/month`, { params: { month: m } }))); fetchSalaryData(); emit('notify', 'Months deleted'); } catch (e) { console.error(e); }
};
...
const missingMonths = computed(() => {
    ...
    const startStr = sorted[0]; const endStr = sorted[sorted.length - 1];
    if (!startStr || !endStr) return [];
    const missing = [];
    let [sy, sm] = startStr.split('-').map(Number); let curr = new Date(sy, sm - 1, 1);
    let [ey, em] = endStr.split('-').map(Number); const last = new Date(ey, em - 1, 1);
    while(curr < last) {
        curr.setMonth(curr.getMonth() + 1); if (curr >= last) break;
        const y = curr.getFullYear(); const m = String(curr.getMonth() + 1).padStart(2, '0');
        const mStr = `${y}-${m}`; if (!existing.has(mStr)) missing.push(mStr);
    }
    return missing;
});
const quickAddMonth = async (mStr) => {
    ...
    fetchSalaryData(); emit('notify', `Created ${mStr}`);
};
...
const loadGenericData = async () => {
    ...
    try {
        ...
        // Reset headers for dragging
        const s = schemas[activeTable.value];
        if(s) {
            editableHeaders.value = s.map(f => ({ title: f.title, key: f.key, sortable: true, width: f.width || '150px' }));
            editableHeaders.value.push({ title: 'Actions', key: 'actions', sortable: false, align: 'end', width: '100px' });
        }
...
const saveGenericItem = async () => {
    ...
    try {
        ...
        dialog.value = false; loadGenericData(); emit('notify', 'Saved successfully');
...
const deleteGenericItem = async (item) => {
    ...
    try { await axios.delete(`${API_URL}/admin/table/${table}/${id}`); loadGenericData(); emit('notify', 'Item deleted'); } catch (e) { emit('notify', 'Delete failed', 'error'); }
};

const openEdit = (item) => { isNew.value = false; editedItem.value = { ...item }; editedId.value = (activeTable.value === 'settings') ? item.key : item.id; dialog.value = true; };
...
watch(() => props.view, (newVal) => {
    if (newVal === 'admin_salary') fetchSalaryData(); else loadGenericData();
}, { immediate: true });

onMounted(() => { if (props.view === 'admin_salary') fetchSalaryData(); else loadGenericData(); });
</script>

<template>
    <div v-if="view === 'admin_salary'">
        <v-alert icon="mdi-shield-account" title="Data Admin" variant="tonal" color="blue-grey" class="mb-6">Review, add, or delete historical monthly records.</v-alert>
        <v-alert v-if="missingMonths.length" type="warning" variant="tonal" class="mb-6 border-s-lg border-warning">
            ...
            <div class="d-flex flex-wrap gap-2"><v-chip v-for="m in missingMonths" :key="m" color="warning" size="small" @click="quickAddMonth(m)" class="font-weight-bold cursor-pointer">{{ m }} <v-icon end icon="mdi-plus-circle" size="small"></v-icon></v-chip></div>
        </v-alert>
        ...
        <v-card>
            <div class="d-flex justify-end px-4 py-2"><v-text-field v-model="salarySearch" label="Search Month" density="compact" variant="plain" hide-details prepend-inner-icon="mdi-magnify" style="max-width: 200px"></v-text-field></div>
            <v-divider></v-divider>
                ...
                <tbody>
                    <tr v-for="item in filteredSalaryRows" :key="item.month" :class="{'bg-blue-grey-lighten-5': editingId===item.month}">
                        <td v-for="col in salaryColumns" :key="col.key" :class="'text-'+col.align">
                            ...
                            <div v-else-if="col.key === 'salary'"><v-text-field v-if="editingId===item.month" v-model.number="editForm.salary" prefix="£" density="compact" variant="outlined" hide-details type="number"></v-text-field><span v-else>£{{ item.salary ? item.salary.toFixed(2) : '0.00' }}</span></div>
                            <div v-else-if="col.key === 'balance'"><v-text-field v-if="editingId===item.month" v-model.number="editForm.balance" prefix="£" density="compact" variant="outlined" hide-details type="number"></v-text-field><span v-else :class="item.balance < 0 ? 'text-red' : 'text-green'">£{{ item.balance ? item.balance.toFixed(2) : '0.00' }}</span></div>
                            <div v-else-if="col.key === 'actions'"><div v-if="editingId===item.month"><v-btn icon="mdi-check" color="green" size="small" variant="text" @click="saveSalaryRow"></v-btn><v-btn icon="mdi-close" color="red" size="small" variant="text" @click="editingId=null"></v-btn></div><v-btn v-else icon="mdi-pencil" color="grey" variant="text" size="small" @click="startEdit(item)"></v-btn></div>
                        </td>
                    </tr>
                </tbody>
            </v-table>
        </v-card>
        <v-card v-if="selectedSalaryRows.length" class="position-fixed bottom-0 left-0 right-0 ma-6 pa-3 rounded-pill bg-inverse-surface d-flex align-center justify-center" style="z-index: 100; max-width: 400px; margin: 0 auto 20px auto !important;"><span class="font-weight-bold mr-4">{{selectedSalaryRows.length}} Selected</span><v-btn icon="mdi-delete" color="error" variant="text" class="mr-2" @click="deleteSalarySelected"></v-btn><v-btn icon="mdi-close" size="small" variant="text" @click="selectedSalaryRows = []"></v-btn></v-card>
    </div>

    <div v-else class="h-100 d-flex flex-column">
        <v-toolbar density="compact" color="transparent" class="mb-2">
            <v-toolbar-title class="text-h6 font-weight-bold text-uppercase text-grey-darken-1">Database: {{ activeTable }}</v-toolbar-title>
            <v-spacer></v-spacer>
        ...
        <v-card class="flex-grow-1" elevation="2">
            <v-data-table :headers="editableHeaders" :items="rawData" :search="search" density="compact" hover fixed-header>
                <template v-slot:headers="{ columns, sort, getSortIcon, toggleSort }">
                    <draggable v-model="editableHeaders" tag="tr" item-key="key" handle=".drag-handle">
                        <template #item="{ element: col }">
                            <th :class="'text-'+(col.align || 'start')" :style="{ width: col.width }" class="resizable-header text-caption text-uppercase" @click="col.sortable && toggleSort(col)">
                                <div class="d-flex align-center" :class="{'justify-end': col.align==='end', 'justify-center': col.align==='center'}">
                                    <v-icon size="small" class="drag-handle cursor-move mr-1" @click.stop>mdi-drag</v-icon>
                                    {{ col.title }}
                                    <v-icon v-if="col.sortable && getSortIcon(col)" size="x-small" class="ml-1">{{ getSortIcon(col) }}</v-icon>
                                </div>
                            </th>
                        </template>
                    </draggable>
        ...
        <v-dialog v-model="dialog" max-width="500px">
            <v-card>
                ...
                <v-card-text class="pt-4"><v-row dense><v-col cols="12" v-for="field in currentFields" :key="field.key"><v-switch v-if="field.type === 'boolean'" v-model="editedItem[field.key]" :label="field.title" color="primary" hide-details :true-value="1" :false-value="0"></v-switch><v-text-field v-else-if="field.readonly && !isNew" v-model="editedItem[field.key]" :label="field.title" readonly variant="filled" density="compact"></v-text-field><v-text-field v-else v-model="editedItem[field.key]" :label="field.title" :type="field.type === 'number' ? 'number' : 'text'" variant="outlined" density="compact"></v-text-field></v-col></v-row></v-card-text>
                <v-card-actions><v-spacer></v-spacer><v-btn variant="text" @click="dialog = false">Cancel</v-btn><v-btn color="primary" variant="elevated" @click="saveGenericItem">Save</v-btn></v-card-actions>
            </v-card>
        </v-dialog>
    </div>
</template>
